...
import { Calendar, Home, Heart, MessageSquare, Search } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      const [bookingsRes, favsRes] = await Promise.all([
        supabase.from('bookings').select('*, properties(title, location, images, price, seller_id)').eq('buyer_id', user.id).order('created_at', { ascending: false }),
                ...
                {bookings.map(b => (
                  <div key={b.id} className="flex items-center gap-4 rounded-xl border p-4">
                    ...
                    <div className="text-right">
                      ...
                      <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${b.status === 'confirmed' ? 'bg-success/10 text-success' : b.status === 'pending' ? 'bg-warning/10 text-warning' : 'bg-muted text-muted-foreground'}`}>
                        {b.status}
                      </span>
                    </div>
                  </div>
            ...
            <div className="max-w-lg space-y-4">
              <div className="space-y-3">
                ...
                >
                  ...
                  {bookings.map(b => (
                    <option key={b.property_id} value={b.property_id}>{b.properties?.title}</option>
                  ))}
                </select>
